...
if (!firebase.apps.length) {
   firebase.initializeApp('golfcard');
}else {
   firebase.app('golfcard'); // if already initialized, use that one
}
...
class App extends React.Component {
  constructor() {
    super();
    this.toggleGameType = (type) => {
      this.setState({ gameType: type });
    };
    this.setHolesList = (list) => {
      this.setState({ holesList: list });
    };
    this.changeHoleView = (hole) => {
    ...
    this.createNewGame = () => {
      let playerNames = this.state.players.map((player) => player.name);
      let postBody = {
      ...
      axios
        .post(
          'http://ec2-54-67-98-244.us-west-1.compute.amazonaws.com:3000/api/games',
          postBody,
        )
        .then((res) => {
          this.setState({
      ...
      <Context.Provider value={this.state}>
        <NavigationContainer>
          <Stack.Navigator>
            ...
            />
            <Stack.Screen name="Holes" component={Holes} />
            <Stack.Screen
              name="HolesView"
              component={HoleView}
              options={{ title: 'Score' }}
