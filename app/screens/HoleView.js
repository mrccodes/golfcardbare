...
const HoleView = ({ navigation, data }) => {
  ...
  useEffect(() => {
    axios
      .get(
        `http://ec2-54-67-98-244.us-west-1.compute.amazonaws.com:3000/api/scoreboard/${data.currentGameId}/${data.currentHole}`,
      )
      .then((res) => {
  ...
  const submitScore = () => {
    ...
    axios
      .post(
        `http://ec2-54-67-98-244.us-west-1.compute.amazonaws.com:3000/api/scoreboard/${gameId}/${hole}`,
        scoreBoard,
      )
      .then((res) => {
        data.updateScoreboard(scoreBoard, numToWord[hole]);
      })
      .catch((err) => {
        console.log(err);
      });
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={HoleViewStyle.span}>
            <TouchableOpacity
              style={HoleViewStyle.dec}
              onPress={() => {
                decrementScore(item.name);
              }}>
              <Text style={HoleViewStyle.btnText}>-</Text>
            </TouchableOpacity>
            <Text style={HoleViewStyle.stats}>
              {item.name}: {item.score}
            </Text>
            <TouchableOpacity
              style={HoleViewStyle.inc}
              onPress={() => {
